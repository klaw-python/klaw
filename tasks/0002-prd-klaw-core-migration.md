...
1. **Clean API**: Establish proper `__all__` exports and `_` prefixed private modules
1. **Documentation**: Fix mkdocs to only generate docs for public API
1. **Future-ready**: Define module structure that accommodates runtime and flight modules
1. **Ecosystem update**: Update all dependent packages to import from `klaw-core`

## 3. User Stories

1. **As a Klaw user**, I want to import Result types from `klaw-core` so that I have one clear foundational package.

1. **As a library consumer**, I want flat imports (`from klaw_core import Ok, Err`) so that my import statements are concise.

1. **As a documentation reader**, I want to see only public API in the docs so that I'm not confused by internal implementation details.

1. **As a Klaw maintainer**, I want a clear module structure so that I can add runtime and flight modules without restructuring.

1. **As a developer**, I want optional extras (`klaw-core[dbase]`) so that I can install only what I need.
...
1. **FR-002**: Move all tests from `klaw-result/tests/` to `klaw-core/tests/`
1. **FR-003**: Merge `pyproject.toml` configurations, preserving all dependencies
1. **FR-004**: Delete `klaw-result` workspace after migration is complete
1. **FR-005**: Update `uv.lock` to reflect the new package structure
...
1. **FR-008**: Each public submodule must have `__all__` defined in its `__init__.py`
...
01. **FR-018**: API docs must NOT include `_internal`, `_sync`, or any `_`-prefixed modules

### 4.6 Dependent Package Updates

19. **FR-019**: Update `klaw-polars` to import from `klaw_core`
01. **FR-020**: Update `klaw-types` to import from `klaw_core` (if applicable)
01. **FR-021**: Update `klaw-testing` to import from `klaw_core` (if applicable)
01. **FR-022**: Update `klaw-plugins` to import from `klaw_core` (if applicable)
01. **FR-023**: Update any examples in `klaw-examples` to use `klaw_core`

### 4.7 Testing

24. **FR-024**: All existing `klaw-result` tests must pass after migration
01. **FR-025**: Import tests must verify both flat and submodule import styles work
01. **FR-026**: Type checking must pass with mypy/pyright in strict mode

## 5. Non-Goals (Out of Scope)

1. **NG-001**: Implementing runtime primitives (channels, tasks, supervision) — separate PRD
1. **NG-002**: Implementing Arrow Flight integration — separate PRD
1. **NG-003**: Adding new functionality to Result/Option types
1. **NG-004**: Publishing to PyPI (internal only for now)
1. **NG-005**: Implementing `[dbase]`, `[polars]`, or other extras — future work
1. **NG-006**: Backwards compatibility shim in `klaw-result` (package will be deleted)
...
1. **SM-002**: `mkdocs build` succeeds without warnings
1. **SM-003**: API documentation shows only public modules (no `_internal`, etc.)
1. **SM-004**: `from klaw_core import Ok, Err, Result, safe` works
1. **SM-005**: `from klaw_core.decorators import safe` works
1. **SM-006**: mypy/pyright pass in strict mode
1. **SM-007**: All dependent packages (`klaw-polars`, etc.) import successfully
1. **SM-008**: `klaw-result` workspace directory is deleted

## 9. Design Decisions (Resolved)

1. **DD-001**: Delete `klaw-result` completely after migration (not published, no shim needed)

1. **DD-002**: Delete all existing `klaw-core` contents (`result.py`, `result_other.py`, `_core.py`, `_another_module.py`) — they are placeholder/dataclass duplicates

1. **DD-003**: Include empty `runtime/` and `flight/` stubs with docstrings pointing to future PRDs

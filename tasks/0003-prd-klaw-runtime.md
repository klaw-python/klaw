...
2. **Result Integration**: All operations return `Result[T, E]`, exceptions are captured
3. **Ergonomic**: `async with` and `async for` patterns throughout
4. **Backend Agnostic**: Same code runs on local processes or Ray—switch via config
5. **Observable**: Structured logging with structlog, context propagation for tracing
6. **Testable**: Real execution tests with Hypothesis and Mimesis, no mocks

## 3. User Stories

1. **As a data engineer**, I want to run CPU-bound tasks in parallel so that I can process large datasets efficiently.

2. **As a backend developer**, I want async producer/consumer channels so that I can build streaming pipelines.

3. **As an ML engineer**, I want the same code to run locally and on a Ray cluster so that I can develop locally and scale to production.

4. **As a developer**, I want context (request_id, trace_id) to propagate through spawned tasks so that I can trace requests across concurrent operations.

5. **As a library user**, I want `runtime.init()` to auto-detect my environment so that I don't need boilerplate configuration.

6. **As a developer**, I want structured logging so that I can integrate with my existing observability stack.
...
2. **FR-002**: Auto-detect environment (CPU count, memory, Ray cluster presence) using psutil
3. **FR-003**: Support environment variables: `KLAW_BACKEND`, `KLAW_CONCURRENCY`, `KLAW_LOG_LEVEL`
4. **FR-004**: Provide `Backend` enum with values: `LOCAL`, `RAY`
5. **FR-005**: Configure structlog with sensible defaults; silent by default, configurable via `log_level` parameter

### 4.2 Executor

6. **FR-006**: Provide `Executor` class as async context manager (`async with Executor() as ex:`)
7. **FR-007**: Executor inherits global config from `runtime.init()`, allows per-instance overrides
8. **FR-008**: Implement `ex.map(func, items, **kwargs)` returning `Result[list[U], E]`
9. **FR-009**: Implement `ex.imap(func, items)` returning `AsyncIterator[Result[U, E]]`
10. **FR-010**: Implement `ex.submit(func, *args, **kwargs)` returning `TaskHandle[T, E]`
11. **FR-011**: Implement `ex.gather(*awaitables)` for concurrent execution
12. **FR-012**: Support per-task backend kwargs override (e.g., `num_cpus`, `num_gpus` for Ray)

### 4.3 TaskHandle

13. **FR-013**: `TaskHandle` is awaitable, returns `Result[T, E | Cancelled]`
14. **FR-014**: Implement `handle.cancel(reason=None)` for explicit cancellation
15. **FR-015**: Implement `handle.is_running()` to check task status

### 4.4 Backends

16. **FR-016**: Implement `LocalBackend` using anyio + aiologic for async, `anyio.to_thread.run_sync()` for CPU-bound
17. **FR-017**: Implement `RayBackend` as optional (requires `ray` extra)
18. **FR-018**: Ray backend passes through kwargs (`num_cpus`, `num_gpus`, `resources`, `runtime_env`)
19. **FR-019**: Local backend ignores Ray-specific kwargs silently
...
21. **FR-021**: Implement `oneshot[T]()` for single-value, single-use channels
22. **FR-022**: Implement `broadcast[T](capacity=None)` for one-to-many (all receivers get every message)
23. **FR-023**: Implement `watch[T](initial)` for latest-value observation
24. **FR-024**: Implement `select(*receivers, timeout=None)` for multiplexing
25. **FR-025**: `Sender` supports `send()` (async, raises on closed), `try_send()` (returns Result), `clone()`, `close()`
26. **FR-026**: `Receiver` supports `recv()` (async, raises on closed), `try_recv()` (returns Result), `clone()`, `__aiter__`
27. **FR-027**: Local channels use `aiologic.Queue`
28. **FR-028**: Distributed channels use msgspec serialization with tagged union framing
29. **FR-029**: Distributed channels enforce backpressure—sender blocks when capacity reached; use `try_send()` for non-blocking with `Result`

### 4.6 Cancellation

30. **FR-030**: Implement `cancel_scope(timeout=None)` as async context manager
31. **FR-031**: Support both scope-based (structured) and handle-based (explicit) cancellation
32. **FR-032**: Cancelled tasks return `Err(Cancelled(reason))`

### 4.7 Timeout & Retry

33. **FR-033**: Implement `timeout(awaitable, seconds)` returning `Result[T, E | Timeout]`
34. **FR-034**: Implement `retry(func, *args, attempts=3, backoff=1.0, retry_on=None)` using tenacity
35. **FR-035**: Retry on `Err` by default, configurable via `retry_on` predicate

### 4.8 Context Propagation

36. **FR-036**: Implement `Context` dataclass with `request_id`, `trace_id`, `span_id`, `extra`
37. **FR-037**: Implement `context(**values)` as async context manager
38. **FR-038**: Implement `current_context()` to retrieve current context
39. **FR-039**: Executor automatically propagates context to spawned tasks via `contextvars`

### 4.9 Structured Logging

40. **FR-040**: Integrate structlog with auto-injection of context (request_id, trace_id)
41. **FR-041**: Log runtime events (task start/end, channel operations, errors) at appropriate levels
42. **FR-042**: Silent by default; configurable via `runtime.init(log_level="info")`
43. **FR-043**: Expose hooks for user customization of logging configuration

### 4.10 Unified Runtime

44. **FR-044**: Implement `Runtime` class as unified wrapper (optional, for convenience)
45. **FR-045**: `Runtime.executor()` returns managed Executor
46. **FR-046**: `Runtime.channel()`, `Runtime.oneshot()`, etc. return channels registered for auto-cleanup
47. **FR-047**: `Runtime.spawn()` spawns tasks managed by runtime
48. **FR-048**: `Runtime.wait()` waits for all spawned tasks
49. **FR-049**: `Runtime.shutdown(wait=True, timeout=30.0)` supports both graceful (wait then cancel) and immediate (`wait=False`) shutdown

### 4.11 Actors & Supervision

50. **FR-050**: Implement `Actor[Msg, Reply]` protocol with `async def handle(self, msg: Msg) -> Reply`
51. **FR-051**: Implement `spawn_actor(actor, capacity=100)` returning `ActorRef[Msg, Reply]`
52. **FR-052**: `ActorRef.send(msg)` for fire-and-forget, `ActorRef.ask(msg)` for request/response returning `Result`
53. **FR-053**: Backend-aware implementation:
    - `Backend.LOCAL`: Channel-based actors with aiologic mailbox
    - `Backend.RAY`: Wraps Ray remote actors (`@ray.remote` classes)
54. **FR-054**: Same `ActorRef` API regardless of backend—user code unchanged between local and Ray
55. **FR-055**: `TaskHandle.exit_reason` property returns `Ok(value) | Err(e) | Cancelled` after completion
56. **FR-056**: `spawn()` and `spawn_actor()` accept `on_exit` callback for lifecycle hooks
57. **FR-057**: Implement restart policies: `restart="never"` (default), `restart="on_failure"`, `restart="always"`
58. **FR-058**: `restart="on_failure"` restarts on `Err`, with configurable `max_restarts` and `restart_delay`
59. **FR-059**: Actors can spawn child actors; parent cancellation cascades to children (linked)
...
61. **FR-061**: Ray actors serialize messages with msgspec; local actors pass by reference

### 4.12 Error Types

62. **FR-062**: All error types are dual: `msgspec.Struct` + `Exception` (can be used in Result or raised)
63. **FR-063**: Implement: `ChannelClosed`, `ChannelFull`, `ChannelEmpty`, `Timeout`, `Cancelled`, `BackendError`, `ActorStopped`, `ActorNotFound`

### 4.13 Persistence & Checkpointing

68. **FR-068**: Implement `Checkpointer` protocol for pluggable persistence
69. **FR-069**: Local backend: use `diskcache` for persistence (default: `~/.klaw/checkpoints/`)
70. **FR-070**: Ray backend: passthrough to Ray's checkpointing and Ray Workflows
71. **FR-071**: `spawn_actor(actor, checkpoint=True, checkpoint_interval=60)` enables actor state checkpointing
72. **FR-072**: Actor state auto-restored on restart from last checkpoint
73. **FR-073**: Implement `Workflow` context manager for checkpointed step execution
74. **FR-074**: `wf.step(name, func, *args)` executes and checkpoints step result
75. **FR-075**: Workflow resumes from last completed step on restart
76. **FR-076**: Checkpoint storage configurable via `runtime.init(checkpoint_path="...")`
77. **FR-077**: State serialized via msgspec for fast, compact storage

### 4.14 Migration

64. **FR-064**: Delete `klaw_core/async_/` directory
65. **FR-065**: Move `AsyncResult` to `klaw_core/runtime/result.py`
66. **FR-066**: Move async itertools to `klaw_core/runtime/itertools.py`
67. **FR-067**: Re-export moved items from `klaw_core/__init__.py` for flat imports
...
2. **NG-002**: Additional backends (Dask, Joblib, Modal) — only Local and Ray supported; no plans for other backends
3. **NG-003**: Custom storage backends for persistence — only diskcache (local) and Ray (distributed) supported
4. **NG-004**: Distributed tracing integration (OpenTelemetry) — future work, hooks provided
5. **NG-005**: GUI or dashboard for monitoring — out of scope
6. **NG-006**: Custom actor scheduler — uses Ray's built-in placement; resources/affinity/placement groups are passthrough only
...
2. **SM-002**: Benchmarks show no regression vs raw anyio (within 5%)
3. **SM-003**: Same code runs locally and on Ray cluster without modification
4. **SM-004**: Context (request_id) propagates correctly through spawned tasks
5. **SM-005**: All channel types pass Hypothesis property-based tests
6. **SM-006**: mypy/pyright pass in strict mode
7. **SM-007**: `runtime.init()` correctly auto-detects environment
...
2. **DD-002**: Distributed channels enforce backpressure—sender blocks when capacity reached; use `try_send()` for non-blocking

3. **DD-003**: No true unbounded channels—default capacity is 10,000; use `unbounded=True` to explicitly opt into unlimited (at user's risk)

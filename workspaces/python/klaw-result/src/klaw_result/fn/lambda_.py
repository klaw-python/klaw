...
if TYPE_CHECKING:
    ...
    from klaw_result.types.result import Ok, Result

__all__ = ["Lambda", "Expr", "Stream", "fn"]
...
class Expr(msgspec.Struct, frozen=True, gc=False):
    ...
    def over(self, iterable: Iterable[Any]) -> Stream:
        ...
        Examples:
            >>> (fn * 2).over([1, 2, 3]).to_list()
            [2, 4, 6]
            >>> (fn * 2).over([1, 2, 3]).filter(fn > 3).to_list()
            [4, 6]
        """
    ...
    def some_if(self, predicate: Callable[[Any], bool]) -> Callable[[Any], Option[Any]]:
        ...
        Examples:
            ...
            Nothing
        """
        from klaw_result.types.option import Nothing, Some
    ...
    def __repr__(self) -> str:
        if not self.ops:
            return "fn"
        ops_str = " -> ".join(
            f"{op}({arg!r})" if arg is not None else op for op, arg in self.ops
        )
        return f"fn[{ops_str}]"
...
class Stream(msgspec.Struct, frozen=True, gc=False):
    ...
    Examples:
        >>> Stream([1, 2, 3]).map(fn * 2).filter(fn > 3).to_list()
        [4, 6]
        >>> Stream([1, 2, 3]).map(fn * 2).sum()
        12
    """
    ...
    def zip(self, *others: Iterable[Any]) -> Stream:
        ...
        return Stream(_iter=zip(self._iter, *others))
    ...
    def filter_some(self) -> Stream:
        ...
        Examples:
            >>> from klaw_result import Some, Nothing
    ...
    def filter_ok(self) -> Stream:
        ...
        Examples:
            >>> from klaw_result import Ok, Err
    ...
    def collect_options(self) -> Option[list[Any]]:
        ...
        Examples:
            ...
            Nothing
        """
        from klaw_result.types.option import Nothing, NothingType, Some
...
class Lambda(Expr):
    ...
    def endswith(
        self, suffix: str | tuple[str, ...], start: int = 0, end: int | None = None
    ) -> Callable[[str], bool]:
    ...
    ) -> Callable[[Iterable[T]], Iterable[tuple[Any, ...]]]:
    ...
    def filter_(
        self, func: Callable[[T], bool] | None = None
    ...
    def iter(self, iterable: Iterable[T]) -> Stream:
        ...
        Examples:
            >>> fn.iter([1, 2, 3]).map(fn * 2).to_list()
            [2, 4, 6]
        """
    ...
    def zip(self, *iterables: Iterable[Any]) -> Stream:
        ...
        Examples:
            ...
            [(1, 'a'), (2, 'b')]
        """
        return Stream(_iter=zip(*iterables))

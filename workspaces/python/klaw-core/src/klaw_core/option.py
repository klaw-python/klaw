...
class Some[T](msgspec.Struct, frozen=True, gc=False):
    ...
    Attributes:
        value: The value of type T contained in this Some.

    Example:
        ```python
        some = Some(42)
        some.unwrap()  # 42
        some.map(lambda x: x * 2)  # Some(value=84)
        with some as value:
            print(value)  # 42
        ```
    """
...
class NothingType(msgspec.Struct, frozen=True, gc=False):
    ...
    def filter[T](self, _predicate: Callable[[T], bool]) -> NothingType:
    ...
    def ok_or[E](self, err: E) -> Err[E]:
        ...
        """
        from klaw_core.result import Err

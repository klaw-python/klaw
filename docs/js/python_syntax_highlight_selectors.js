document.addEventListener("DOMContentLoaded", () => {
	...
	const keywords = document.querySelectorAll(
		".doc-signature .k, .md-code__content .k",
	);
	...
	// Functions
	const funcDefs = document.querySelectorAll(
		".doc-signature .nf, .md-code__content .nf",
	);
	...
	// Classes
	const classDefs = document.querySelectorAll(
		".doc-signature .nc, .md-code__content .nc",
	);
	...
	const decorators = document.querySelectorAll(
		".doc-signature .nd, .md-code__content .nd",
	);
	...
	const attrNames = document.querySelectorAll(
		".doc-signature .na, .md-code__content .na",
	);
	attrNames.forEach((span) => {
		if (!span.closest(".sd")) {
			...
			while (
				...
				previousNonWhitespace.textContent.trim() === ""
			) {
	...
	const sigParams = document.querySelectorAll(
		".doc-signature .n, .md-code__content .n",
	);
			...
			if (!isAfterArrow) {
				...
				if (span.closest(".doc-signature")) {
					...
					while (contextCheck && contextCheck !== document) {
						if (
							contextCheck.querySelector &&
							contextCheck.querySelector(".nf")
						) {
	...
	const arrows = document.querySelectorAll(
		".doc-signature .o, .md-code__content .o",
	);
	arrows.forEach((span) => {
		if (span.textContent.trim() === "->") {
			let nextElement = span.nextElementSibling;
			const inReturnType = true;
			let typeDepth = 0;
			...
			while (nextElement && inReturnType) {
				...
				if (
					nextClasses.contains("w") ||
					nextClasses.contains("p") ||
					nextText === ""
				) {
					nextElement = nextElement.nextElementSibling;
					continue;
				...
				if (
					nextText === ":" ||
					nextText === ")" ||
					nextText === "," || // Parameter boundaries
					nextClasses.contains("k") ||
					nextClasses.contains("nf") ||
					nextClasses.contains("nc") || // Keywords, function/class defs
					typeDepth < 0
				) {
					break;
				}
				...
				if (
					nextClasses.contains("n") ||
					nextClasses.contains("nb") ||
					nextClasses.contains("nc")
				) {
					...
					if (
						isGenericParam &&
						!nextElement.hasAttribute("data-type-annotation")
					) {
	...
	attributes.forEach((span) => {
		if (!span.closest(".sd")) {
			...
			if (
				...
				if (
					!functionNames.has(span.textContent.trim()) &&
					!methodNames.has(span.textContent.trim())
				) {
		...
		if (!span.closest(".sd")) {
			...
			if (typingClasses.includes(text)) {
				...
				for (let i = 0; i < 8 && prev.previousElementSibling; i++) {
					...
					if (
						prev.classList.contains("nn") &&
						prev.textContent.trim() === "typing"
					) {
						foundTyping = true;
					}
					if (
						prev.classList.contains("kn") &&
						prev.textContent.trim() === "from"
					) {
						foundImport = true;
					}
	...
	const importStatements = document.querySelectorAll(
		".doc-signature .n, .md-code__content .n",
	);
		...
		if (!span.closest(".sd")) {
			...
			if (
				...
				while (moduleCheck.previousElementSibling) {
					...
					if (
						moduleCheck.classList.contains("kn") ||
						moduleCheck.textContent.trim() === "from"
					) {
						break;
					}
				}
				...
				if (
					...
					!text.match(
						/^(List|Dict|Any|Optional|Union|Tuple|Set|Dict|Callable|Type|Literal)$/,
					)
				) {
					// Check if it's from a known module that contains functions
					if (
						moduleContext === "klaw_types" ||
						moduleContext.match(/^[a-z_][a-z0-9_]*$/) || // Python module
						!moduleContext.match(/^(typing|collections)$/)
					) {
						// Not from typing/collections
	...
	const typeAnnotations = document.querySelectorAll(
		".doc-signature .n, .md-code__content .n, .doc-signature .nb, .md-code__content .nb",
	);
	typeAnnotations.forEach((span) => {
		if (!span.closest(".sd")) {
			...
			if (
				...
				while (
					contextCheck &&
					contextCheck !== document &&
					contextDistance < maxContextDistance
				) {
					if (
						contextCheck.querySelector(".nf") || // Function name
						(contextCheck.classList.contains("k") &&
							contextCheck.textContent.trim() === "def")
					) {
				...
				if (functionContext) {
					...
					if (
						...
						while (nestedCheck && nestedCheck !== document) {
							...
							if (
								(nestedCheck.classList.contains("n") ||
	...
	names.forEach((span) => {
		if (!span.closest(".sd")) {
			...
			// Then check for actual parameters (only if not tagged as variable)
				...
				(functionNames.has(text) || methodNames.has(text)) &&

...
(function() {
    ...
    function initializeFolding() {
        ...
        const codeBlocks = document.querySelectorAll('.md-content .md-code__content');
        ...
        codeBlocks.forEach(function(codeBlock, blockIndex) {
            ...
            const lineSpans = Array.from(codeBlock.querySelectorAll('[id^="__span-0-"]'));
            ...
            while (i < lineSpans.length) {
                ...
                if (openingMatch) {
                    ...
                    console.log(`  Found potential docstring opening at line ${i + 1}: indent=${indentSpaces}, "${trimmedText.substring(0, 30)}..."`);
                    
                    // Check if single-line (closing """ on same line)
                    if (trimmedText.includes('"""', 4)) { // Has closing after opening
                    ...
                    for (let j = i + 1; j < lineSpans.length; j++) {
                        ...
                        if (testMatch && testMatch[1].length === indentSpaces) {
                            endIndex = j;
                            console.log(`  Found docstring closing at line ${j + 1} (indent ${indentSpaces})`);
                            break;
                        }
                    }
                    ...
                    if (endIndex !== -1 && endIndex > i) {
                        ...
                        const hasReasonableLength = numContentLines > 0 && numContentLines < 100;

                        if (hasReasonableLength) {
                            console.log(`  Valid docstring: lines ${i + 1}-${endIndex + 1} (${candidateLines.length} lines, ${numContentLines} content)`);
                            createDocstringFold(codeBlock, candidateLines, i);
                            totalFolds++;
                            i = endIndex + 1;
                        } else {
                            console.log(`  Invalid length - skipping (${numContentLines} content lines)`);
                            i++;
                        }
                    } else {
                        console.log(`  No matching closing found for opening at line ${i + 1}`);
                        i++;
                    }
                } else {
                    i++;
        ...
        console.log(`Created ${totalFolds} docstring folds, skipped ${singleLinesSkipped} single-lines`);
    }
    ...
    function createDocstringFold(codeBlock, docstringLines, startIndex) {
        ...
        if (lineno) {
            ...
            lineno.addEventListener('keydown', function(e) {
        ...
        for (let k = 1; k < docstringLines.length; k++) {
            ...
            if (lineClone.id) {
                lineClone.id = lineClone.id.replace(/__span-0-\d+$/, `docstring-line-${startIndex + k}`);
            }
        ...
        console.log(`  Fold inserted for lines ${startIndex + 1}-${startIndex + docstringLines.length}`);
    }
    ...
    function toggleFold(foldContainer, indicator) {
        ...
        if (isExpanded) {
            ...
            if (indicator) indicator.textContent = indicator.textContent.replace(' ', ' ');
    ...
    function init() {
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', () => setTimeout(initializeFolding, 100));
        } else {
    ...
    const observer = new MutationObserver(() => setTimeout(initializeFolding, 200));
